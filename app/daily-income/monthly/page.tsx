...
interface MonthlyStats {
  ...
  maxNet: { date: string; amount: number };
  minNet: { date: string; amount: number };
  dailyData: DailyData[];
}
      ...
      {loading ? (
        ...
        <div>
          ...
          <div style={{
            ...
            <StatCard title="총 일반 매출" value={formatKRW(stats.totalIncome)} bgColor="#e0f2fe" textColor="#0369a1" />
            <StatCard title="총 본부/조제(차감)" value={formatKRW(stats.totalExpense)} bgColor="#fef3c7" textColor="#92400e" />
            <StatCard title="총 정산 합계" value={formatKRW(stats.totalNet)} bgColor="#d1fae5" textColor="#047857" />

            <StatCard title="총 POS 매출" value={formatKRW(stats.totalPos)} bgColor="#ede9fe" textColor="#4c1d95" />
            <StatCard
              title="총 차액"
              value={formatKRW(stats.totalDiff)}
              bgColor={stats.totalDiff >= 0 ? "#dcfce7" : "#fee2e2"}
              textColor={stats.totalDiff >= 0 ? "#166534" : "#b91c1c"}
            />
            <StatCard
              title="기록된 일수"
            ...
            <StatCard title="일 평균 일반 매출" value={formatKRW(Math.round(stats.avgIncome))} bgColor="#dbeafe" textColor="#1e40af" />
            <StatCard title="일 평균 차감" value={formatKRW(Math.round(stats.avgExpense))} bgColor="#fee2e2" textColor="#b91c1c" />
            <StatCard title="일 평균 정산 합계" value={formatKRW(Math.round(stats.avgNet))} bgColor="#dcfce7" textColor="#166534" />

            <StatCard
              title="최고 정산 (Max Settlement)"
              value={`${stats.maxNet.date.substring(5)} (${formatKRW(stats.maxNet.amount)})`}
              bgColor="#e0e7ff"
              textColor="#3730a3"
            />
            <StatCard
              title="최저 정산 (Min Settlement)"
              value={`${stats.minNet.date ? stats.minNet.date.substring(5) : 'N/A'} (${formatKRW(stats.minNet.amount)})`}
              bgColor="#ede9fe"
              textColor="#5b21b6"
            />
            <StatCard
            ...
            <table style={{
              ...
              <thead>
                <tr style={{
                  ...
                  <th style={{ padding: '0.75rem', borderBottom: '1px solid #e5e7eb' }}>일반 매출</th>
                  <th style={{ padding: '0.75rem', borderBottom: '1px solid #e5e7eb' }}>본부/조제(차감)</th>
                  <th style={{ padding: '0.75rem', borderBottom: '1px solid #e5e7eb' }}>정산 합계</th>
