-- 간단한 PIN 시스템: 고객명 + PIN 조합 로그인 (최종 수정됨)
...
-- 6. 고객명 + PIN 인증 함수 (파라미터 이름 수정)
CREATE OR REPLACE FUNCTION authenticate_customer_by_name_and_pin(
    input_customer_name VARCHAR, 
    input_pin VARCHAR(6)
)
RETURNS TABLE(
    customer_id UUID,
...
BEGIN
    ...
    SELECT 
        ...
        c.is_initial_pin
    FROM customers c
    WHERE c.name = input_customer_name
        AND c.pin_code = input_pin 
        AND c.is_deleted = FALSE;
END;
$$ LANGUAGE plpgsql;

-- 7. PIN 변경 함수 (고객 ID 기반)
CREATE OR REPLACE FUNCTION change_customer_pin(
    input_customer_uuid UUID, 
    input_new_pin VARCHAR(6)
)
RETURNS BOOLEAN AS $$
BEGIN
    -- PIN 변경 (중복 체크 없음)
    UPDATE customers 
    SET 
        pin_code = input_new_pin,
        is_initial_pin = FALSE,
        pin_updated_at = now(),
        updated_at = now()
    WHERE id = input_customer_uuid 
...
CREATE OR REPLACE FUNCTION reset_customer_pin(input_customer_uuid UUID)
RETURNS BOOLEAN AS $$
BEGIN
    UPDATE customers 
    SET 
...
CREATE OR REPLACE FUNCTION search_customer_by_name(input_search_name VARCHAR)
RETURNS TABLE(
    customer_id UUID,
    customer_code VARCHAR,
    customer_name VARCHAR,
...
BEGIN
    ...
    SELECT 
        ...
        c.phone
    FROM customers c
    WHERE c.name ILIKE '%' || input_search_name || '%'
        AND c.is_deleted = FALSE
    ORDER BY c.name
    LIMIT 10;
END;
